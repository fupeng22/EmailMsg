...
function onStopDrag(e) {
    ...
    $.ajax({
        type: "GET",
        url: "MapPotUpdatePosition.ashx?posX=" + encodeURI(txtMapPortXPos_Add_val) + "&posY=" + encodeURI(txtMapPortYPos_Add_val) + "&MapPotId=" + encodeURI(mapPotId),
        data: "",
        async: true,
        cache: false,
...
function onStopResize(e) {
    ...
    $.ajax({
        type: "GET",
        url: "MapPotUpdateSize.ashx?Width=" + encodeURI(txtMapPortWidth_val) + "&Height=" + encodeURI(txtMapPortHeight_val) + "&MapPotId=" + encodeURI(mapPotId),
        data: "",
        async: true,
        cache: false,
        beforeSend: function (XMLHttpRequest) {
...
$(function () {
    ...
    $("#mnu_AddMapPort").click(function (e) {
        ...
        QueryAvialableEquipmentByMapIdURL = "GetAvailableEquipmentByGroupId.ashx?hid_MapId=" + encodeURI($("#hid_MapId").val());
        $('#CGEquipmentForMapPort_Add').combogrid({
            ...
            columns: [[
					...
					{ field: 'EquipmentName', title: '设备名称', width: 270 }
				]]
        });
    ...
    //                                    onStopDrag(e);
    //                                }
    //                            }).css("width", mapPotsInfo[i].Width).css("height", mapPotsInfo[i].Height).resizable({
    //                                minWidth: 30,
    //                                minHeight: 40,
    //                                maxWidth: 300,
    //                                maxHeight: 400,
    ...
    //    document.getElementById("body_Main").oncontextmenu = function (e) {
    //        alert("http://localhost:4789/admin/map/mapmain.aspx");
    ...
    $("#mnu_UpdateMapPort_Detail").click(function () {
        ...
        if (!bSele) {
            $.messager.alert("信息提示", "请选择需要修改的设备点", "error");
            return false;
        }
        ...
        QueryAvialableEquipmentByMapIdURL = "GetAvailableEquipmentByGroupId_Update.ashx?hid_MapId=" + encodeURI($("#hid_MapId").val()) + "&EquipmentId=" + encodeURI(EquipmentId);
        ...
        MapAdd_dlg = $('#div_MapPotUpdate').dialog({
            buttons: [{
                ...
                handler: function () {
                    ...
                    $.ajax({
                        type: "GET",
                        url: "MapPotUpdate.ashx?MapEquipmentId=" + encodeURI(CGEquipmentForMapPort_Update_val) + "&MapId=" + encodeURI($("#hid_MapId").val()) + "&MapPotName=" + encodeURI(txtMapPortCustomizeName_Update_val) + "&posX=" + encodeURI(txtMapPortXPos_Update_val) + "&posY=" + encodeURI(txtMapPortYPos_Update_val) + "&Width=" + encodeURI(txtMapPortWidth_Update_val) + "&Height=" + encodeURI(txtMapPortHeight_Update_val) + "&MapPotId=" + encodeURI(mapPotId),
                        data: "",
                        async: true,
                        cache: false,
                        beforeSend: function (XMLHttpRequest) {
    ...
    $("#mnu_DeleMapPort_Detail").click(function () {
        ...
        if (!bSele) {
            $.messager.alert("信息提示", "请选择需要删除的设备点", "error");
            return false;
        }

        $.ajax({
            type: "GET",
            url: "MapPotDele.ashx?mapPotId=" + encodeURI(mapPotId),
            data: "",
            async: true,
            cache: false,
            beforeSend: function (XMLHttpRequest) {
