function ResetAlarmToNormal(mapPotId, ReceiveMsgId) {
    //后台更改为已阅读
    $.ajax({
        type: "GET",
        url: "MapPotView.ashx?ReceiveMsgId=" + encodeURI(ReceiveMsgId),
        data: "",
        async: true,
        cache: false,
        ...
        success: function (msg) {
            ...
            if (JSONMsg.result.toLowerCase() == 'ok') {
                //复原图标为蓝色
                $("#imgAlarmPot_" + mapPotId).attr("class", $("#imgAlarmPot_" + mapPotId).attr("normalCls"));
                //并更新此提示信息内容
                ...
                $.messager.alert("提示信息", JSONMsg.message, "info");
            } else {
...
$(function () {
    ...
    function RefreshAlarmInfo() {
        $.ajax({
            type: "GET",
            url: "MapMain.ashx?type=3&MapId=" + encodeURI($("#hid_MapId").val()),
            data: "",
            async: true,
            cache: false,
            beforeSend: function (XMLHttpRequest) {
            ...
            success: function (msg) {
                var JSONMsg = eval("(" + msg + ")");
                if (JSONMsg.AlarmsPots.length > 0) {
                    for (var i = 0; i < JSONMsg.AlarmsPots.length; i++) {
                        $("#imgAlarmPot_" + JSONMsg.AlarmsPots[i].PotId).attr("class", "mpCur iconMpCur");
                        $("#imgAlarmPot_" + JSONMsg.AlarmsPots[i].PotId).attr("ReceiveMsgId", JSONMsg.AlarmsPots[i].ReceiveMsgID);

                        $('#span_MapPotSet_' + JSONMsg.AlarmsPots[i].PotId).tooltip({
                            content: function () {
                                var bOK = false;
                                var strRet = "";
                                ...
                                $.ajax({
                                    ...
                                    success: function (msg) {
                                        var JSONmsg = eval("(" + msg + ")");
                                        strRet = "<table>" + "<tr><td><input type='button' onclick='javascript:ResetAlarmToNormal(" + JSONmsg[0].Id + "," + $("#imgAlarmPot_" + JSONmsg[0].Id).attr("ReceiveMsgId") + ")" + "' value='报警复位'/></td></tr>"
