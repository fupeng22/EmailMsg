$(function () {
    ...
    _$_datagrid.datagrid({
        ...
        idField: 'Id',
        columns: [[
					{ field: 'MapName', title: '地图名称', width: 90, sortable: true,
					    sorter: function (a, b) {
					        return (a > b ? 1 : -1);
					    }
					},
                    { field: 'GroupName', title: '所在区域', width: 80, sortable: true,
                        sorter: function (a, b) {
                            return (a > b ? 1 : -1);
                        }
                    }
    ...
    $("#btnAddMap").click(function () {
        ...
        if (GroupIdSele == -1) {
            $.messager.alert("提示信息", "请先选择地图所在区域", "error");
            return false;
        }
                    ...
                    $('#form_MapAdd').form('submit', {
                        ...
                        onSubmit: function () {
                            if ($("#txtMapName_Add").val() == "" || $("#mapFile_Add").val() == "") {
                                $.messager.alert("信息提示", "请填写完整信息(地图名称、地图文件)", "error");
                                return false;
                            }
                        ...
                        success: function (data) {
                            ...
                            if (msg.result == "ok") {
                                ...
                                $.messager.alert("信息提示", msg.message, "info");
                            } else {
                                $.messager.alert("信息提示", msg.message, "error");
                                return false;
                            }
    ...
    $("#btnUpdateMap").click(function () {
        ...
        var obj = getMapSele();
        if (obj[1] == -1) {
            $.messager.alert("提示信息", "请先选择地图", "error");
            return false;
        }
